...
interface FriendScreensProps {
  currentScreen: ScreenType;
  onNavigate: (screen: ScreenType) => void;
}

const FriendScreens: React.FC<FriendScreensProps> = ({ currentScreen, onNavigate }) => {
    ...
    <div className="flex flex-col h-full bg-background-dark animate-fade-in-up">
      <header className="px-6 pt-10 pb-4 sticky top-0 bg-background-dark/95 backdrop-blur-md z-20">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-4xl font-extrabold text-white tracking-tight font-display">Friends</h1>
          <div className="flex items-center gap-4">
            <button className="w-12 h-12 rounded-full bg-white/5 flex items-center justify-center text-white">
              <span className="material-symbols-outlined">qr_code_scanner</span>
            </button>
            <button className="w-12 h-12 rounded-full bg-primary flex items-center justify-center text-white shadow-xl shadow-primary/30">
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full h-14 bg-card-dark border-none rounded-2xl pl-12 pr-4 text-white placeholder:text-gray-700" 
          />
        </div>
      </header>
      ...
      <main className="flex-1 px-4 space-y-8 overflow-y-auto scrollbar-hide pb-24">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[10px] font-bold text-gray-600 uppercase tracking-[0.2em]">Requests (2)</h3>
            <button onClick={() => onNavigate(ScreenType.FRIEND_REQUESTS)} className="text-[10px] font-bold text-primary uppercase">View All</button>
          </div>
          <div className="space-y-3">
             <div className="p-4 bg-card-dark rounded-3xl border border-white/5 flex items-center gap-4">
                <div className="relative">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest">Wants to share location</p>
                </div>
                <div className="flex gap-2">
                   <button className="w-9 h-9 bg-white/5 rounded-xl flex items-center justify-center text-gray-500"><span className="material-symbols-outlined text-xl">close</span></button>
                   <button className="px-4 bg-primary rounded-xl text-white text-xs font-bold shadow-lg shadow-primary/10">Accept</button>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="space-y-2 pt-2">
          <h3 className="px-2 text-[10px] font-bold text-gray-600 uppercase tracking-[0.2em] mb-4">All Friends (42)</h3>
          {[
            ...
            <div key={i} className="flex items-center gap-4 p-4 hover:bg-white/5 transition-all cursor-pointer border-b border-white/5 last:border-0 group">
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <button className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center"><span className="material-symbols-outlined text-lg">chat_bubble</span></button>
                <button className="w-10 h-10 rounded-full text-gray-600 flex items-center justify-center"><span className="material-symbols-outlined text-lg">more_vert</span></button>
              </div>
            </div>
          ))}
        </div>
