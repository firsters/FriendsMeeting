...
const Auth = ({ type = 'login', onBack, onSuccess }) => {
  ...
  return (
    <div className="relative flex min-h-full w-full flex-col justify-between overflow-x-hidden max-w-md mx-auto bg-white text-[#212121] font-sans antialiased">
      {/* Main Content Wrapper */}
      <div className="flex flex-col flex-1">
        {/* Header Section */}
        <div className="pt-10 pb-6 relative px-4 text-center">
          <div 
          ...
          <div className="flex justify-center mb-6 relative z-10">
            <div className="w-16 h-16 bg-[#4285F4] rounded-2xl flex items-center justify-center shadow-lg shadow-[#4285F4]/20">
              <span className="material-symbols-outlined text-white text-3xl">location_on</span>
            </div>
          </div>

          <div className="relative z-10 flex flex-col gap-2">
            <h1 className="text-[#212121] tracking-tight text-2xl font-bold leading-tight">
        ...
        {/* Login Form Area */}
        <div className="px-6 py-4 flex flex-col gap-6 relative z-10">
          <form className="flex flex-col gap-5" onSubmit={(e) => { e.preventDefault(); onSuccess(); }}>
            {!isLogin && (
              <div className="flex flex-col gap-2">
                <label className="text-[#212121] text-sm font-semibold">{t('auth_nickname')}</label>
                <div className="relative flex items-center">
                  <span className="absolute left-4 text-[#757575]">
                    <span className="material-symbols-outlined text-[20px]">person</span>
                  </span>
                  <input 
                    className="flex w-full rounded-lg text-[#212121] focus:outline-none focus:ring-1 focus:ring-[#4285F4] focus:border-[#4285F4] border border-[#E0E0E0] bg-white h-12 pl-11 pr-4 text-sm font-normal placeholder-[#BDBDBD] transition-all"
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[#212121] text-sm font-semibold">{t('auth_email') || 'Email or Username'}</label>
              <div className="relative flex items-center">
                <span className="absolute left-4 text-[#757575]">
                  <span className="material-symbols-outlined text-[20px]">person</span>
                </span>
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[#212121] text-sm font-semibold">{t('auth_password') || 'Password'}</label>
              <div className="relative flex items-center">
                <span className="absolute left-4 text-[#757575]">
                  <span className="material-symbols-outlined text-[20px]">lock</span>
                </span>
                <input 
                  className="flex w-full rounded-lg text-[#212121] focus:outline-none focus:ring-1 focus:ring-[#4285F4] focus:border-[#4285F4] border border-[#E0E0E0] bg-white h-12 pl-11 pr-12 text-sm font-normal transition-all placeholder-[#BDBDBD]"
                  placeholder={t('auth_password_placeholder') || 'Enter your password'}
                  type={showPassword ? 'text' : 'password'}
                />
                <button 
                  className="absolute right-0 top-0 bottom-0 px-4 flex items-center justify-center text-[#757575] hover:text-[#4285F4] transition-colors"
            ...
            {isLogin && (
              <div className="flex justify-end mt-[-8px]">
                <button 
                  className="text-[#4285F4] text-xs font-semibold hover:underline transition-all"
            ...
            <button className="flex w-full cursor-pointer items-center justify-center overflow-hidden rounded-lg h-12 bg-[#4285F4] text-white text-base font-bold leading-normal tracking-[0.015em] hover:bg-[#3367d6] active:scale-[0.98] transition-all shadow-md shadow-[#4285F4]/10 mt-2">
      ...
      {/* Footer Section */}
      <div className="px-6 py-8 flex flex-col items-center gap-4 relative z-10 border-t border-[#F5F5F5]">
        <p className="text-[#757575] text-sm font-normal text-center">
          ...
          <button 
            className="text-[#4285F4] font-semibold hover:underline ml-1"
        ...
        <div className="flex items-center gap-1.5 opacity-60">
          <span className="material-symbols-outlined text-[16px] text-[#757575]">lock</span>
          <span className="text-[#757575] text-xs font-semibold uppercase tracking-wider">Secured connection</span>
        </div>
      </div>
    </div>
  );
